...
=======
We can simply add the score-function estimator and subtract the reparameterization estimator, to produce a gradient estimator which we call LAX:
>>>>>>> e27663857b6181d480c0ab7611c52307b388680e
%
\begin{align}
\label{eq:cont_est}
...
<<<<<<< HEAD
The work most related to ours is the recently-developed REBAR method~\citep{tucker2017rebar}.
...
While sometimes true, these are strong assumptions to make.
Furthermore, REBAR and concrete require that the function being optimized is known so they are inapplicable for optimizing black-box functions, as in reinforcement learning settings where the reward is an unknown function of the environment.
=======
%Unfortunately, REBAR and concrete require the function being optimized, whose input is only defined at discrete inputs, to also accept continuous inputs, be differentiable w.r.t. those inputs, and behave predictably with respect to those continuous inputs.
...
However, a dependence on $\theta$ can when training probabilisitic models, or when we add a regularizer to a black-box optimization problem.% or as a regularizer (such as weight-decay).
In both these settings, if the dependence on $\theta$ is known and differentiable, we can use the fact that
%If $f$ does depend on $\theta$ we note:
\begin{align}
\PT \E_{p(b|\theta)}[f(b, \theta)] = \E_{p(b|\theta)}\left[\PT f(b, \theta) + f(b, \theta)\PT \log p(b|\theta) \right]
\end{align}
%The second term on the right-hand side can be estimated using any of the methods presented in this paper and 
%If the dependence on $\theta$ is known and differentiable, for instance if   then the left-hand side can be computed by back-propagation.
and simply add the term $\PT f(b, \theta)$ to our estimator.
...
\end{wrapfigure}
We seek to minimize $\mathbb{E}_{p(b|\theta)}[(b - t)^2]$ as a function of the parameter $\theta$ where {$p(b|\theta) = \textnormal{Bernoulli}(b|\theta)$}.
...
Training curves for all models can be seen in figure~\ref{fig:vae curves}.
...
\begin{figure}[h]
\centering
\includegraphics[width=.22\textwidth]{figures/MNIST_L1}
\includegraphics[width=.22\textwidth]{figures/MNIST_L2}
\includegraphics[width=.22\textwidth]{figures/OMNIGLOT_L1}
\includegraphics[width=.22\textwidth]{figures/OMNIGLOT_L2}
\caption{Training curves for VAE Experiments. Left to right: MNIST 1 Layer, MNIST 2 Layer, Omniglot 1 Layer, Omniglot 2 Layer. The horizontal dashed line indicates the lowest validation error obtained by REBAR.}
\label{fig:vae curves}
\end{figure}
...
In the discrete action setting, we test our approach on the Cart Pole and Lunar Lander environments as provided by the OpenAI gym~\citep{1606.01540}. In the continuous action setting, we test on the MuJoCo~\citep{todorov2012mujoco}-simulated environments Inverted Pendulum and Inverted Double Pendulum also found in the OpenAI gym.

In all tested environments we observe improved performance and sample efficiency using our method. The results of our experiments can be seen in figure~\ref{fig:rl_results}, [CITE OTHER RELEVANT FIGURES] and table~\ref{tab:rl_results}.

We found that our estimator produced policy gradients with drastically reduced variance (see figure~\ref{fig:rl_var}) allowing for larger learning rates to be used while maintaining stable training. In both discrete environments our estimator achieved an over 2-times speedup in convergence over the baseline.
...
\includegraphics[width=.25\linewidth]{figures/cp_paper_test} & 
\includegraphics[width=.25\linewidth]{figures/ll_paper_test} &
\includegraphics[width=.25\linewidth]{figures/cp_paper_test} & 
\includegraphics[width=.25\linewidth]{figures/cp_paper_test}\\
 \end{tabular}
...
\begin{figure}[h]
\centering
\includegraphics[width=.22\textwidth]{figures/cp_paper_var}
\includegraphics[width=.22\textwidth]{figures/cp_paper_var}
\includegraphics[width=.22\textwidth]{figures/cp_paper_var}
\includegraphics[width=.22\textwidth]{figures/cp_paper_var}
\caption{Variance of policy gradients (log scale). Left to right: Cart Pole, Lunar Lander, Inverted Pendulum, Inverted Double Pendulum. In each curve, the center line indicates the mean reward over 5 random seeds. The opaque bars indicate 1 standard deviation. Our model is in orange and the baseline is in blue. After every 10th training episode 100 episodes were run and the sample log-variance is reported averaged over all policy parameters}
\label{fig:rl_var}
\end{figure}
...
For example, we could apply our estimator to train a VAE with continuous latent variables who's generative model is non-differentiable (a rendering engine perhaps).
We also feel that there is much room to explore model design choices for the control variate and develop a greater understand the properties of the optimal control variate. 
...
=======
We believe our results in reinforcement learning are quite promising and should motivate further research into using action-dependent control-variates for policy-gradient methods.
...
We also feel that the relationship between our learned control variate and the action-value function (commonly denoted as $Q$) is worth exploring and understanding in greater detail than we do now. 
