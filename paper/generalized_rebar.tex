...
\emph{Right:} The relaxation being used by REINFORCE (red), REBAR (blue), and the RELAX's learned relaxation (green).
}
\label{first figure}
\end{figure}
...
%While these assumptions often hold, there are many problems of interest which make these approaches inapplicable. 
...
\end{align}
%
We assume that $f$ cannot be differentiated through.
...
\subsection{Conditional reparameterization}
%When $p(b|\theta)$ is a distribution over discrete random variables, more care must be taken.
We assume that there exists a continuous, reparameterizable distribution $p(z|\theta)$ and a deterministic mapping $H(z)$ such that $H(z) = b \sim p(b|\theta)$ when $z \sim p(z|\theta)$.
Examples of such distributions and functions can be found in appendix~\ref{resample}.
%Most discrete distributions of interest fit into this formulation.

We derive our estimator similarly to \cite{tucker2017rebar} with
%
\begin{align}
\PT \expectedLoss{} %\nonumber\\
...
\end{align}
%
which is also unbiased for any $r_\phi$.
...
Inspecting the square of \eqref{eq:cont_est}, we can see that this loss encourages $r_\phi(b)$ to approximate $f(b)$, but with a weighting based on $\PT\log p(b)$.  % Todo: reword this awkward sentence.
...
%Our approach allows us to design our control variate $r_\phi$ in any way we please.
...
\section{Reinforcement learning with \LAX{} and \RELAX{}}
We now describe how we apply the \LAX{} and \RELAX{} estimators in the reinforcement learning (RL) setting.
By reinforcement learning, we refer to the problem of optimizing the parameters $\theta$ of a policy distribution $\pi(a | s, \theta)$ to maximize the sum of rewards~\citep{sutton2000policy}.
...
The function whose expectation is being optimized, $R$, maps $\tau$ to the sum of rewards
...
This problem can be approached by estimating the gradients of an expectation of a black-box function: $\frac{\partial \mathbb{E}_{p(\tau|\theta)}[R(\tau)]}{\partial \theta}$.
The \emph{de facto} standard approach is the advantage actor-critic estimator: [cite]% which is the standard policy gradient algorithm. 
%We seek to compute
%
%\begin{align}
...
%
\begin{align}
\hat g_{\textnormal{A2C}} = \sum_{t=1}^T \LL{t} \left[ \sum_{t'=t}^T r_{t'} - b(s_t) \right]+\frac{\partial}{\partial\theta} b(s_t), \\
...
This estimator is unbiased when $b$ does not depend on $a_t$. 

How to apply \RELAX{} in this setting?
...
To construct this estimator, we first observe that $\forall t$, we have
%
\begin{align}
...
\begin{align*}
\PT\E_{p(\tau)}[f(\tau)]
%\E_{p(\tau)}\left[f(\tau)\cdot\LP{\tau;\theta}\right]-\sum_t\E_{p(\tau)}\Big[\LL{t} r_\phi(a_t, s_t)\Big]+\\&\sum_t \E_{p(a_{1:t-1},s_{1:t})}\Big[\frac{\partial}{\partial\theta}\E_{\pi(a_t|s_t, \theta)}\Big[r_\phi(a_t,s_t)\Big]\Big]\nonumber\\
...
Comparing $\hat g_\LAX^{\RL}$ to the standard advantage actor-critic estimator in \eqref{eq:rl_a2c}, the main difference being that our baseline is action-dependent, without introducing bias.
Details of this derivation, and the discrete version of the estimators can be found in the appendix.
