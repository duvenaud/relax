...
%, the most notable of which being the use of deep neural networks as general purpose function approximators.
Through the back-propagation algorithm \citep{rumelhart1985learning}, the gradient of a loss function can be obtained with respect to each model parameter and learning can be accomplished via gradient decent.
Recent advancements such as Stochastic Variational Inference \citep{hoffman2013stochastic} and the reparameterization trick \citep{kingma2013autoencoding, rezende2014stochastic} have enabled the use of gradient-based learning for scalable, approximate inference in large probabilistic latent-variable models.
% enabling learning in vary large probabilistic models with complex approximate posterior distributions.
% [ cite NORMALIZING FLOWS OR SOMETHING].
%Unfortunately, these approaches are limited to models with continuous random variables or.
...
The situation is complicated by the presence of discrete random variables or black-box functions. [cite stochasic computation graphs, and black-box variational inference paper] 
...
\subsubsection{Reducing Gradient Variance through Control Variate}
