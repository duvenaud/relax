...
%\algtext*{EndFunction}\dfrac{num}{den}
...
\newcommand{\LPT}[1]{\PT \log p(#1|\theta)}
...
\begin{algorithm}[H]
	\caption{RELAX Gradient Estimator for Continuous Random Variables}
...
We wish to compute $\PT\E_{p(b|\theta)}[f(b)]$ where $p(b|\theta)$ is a distribution with continuous support that admits a reparameterization $T$ such that $T(\theta, \epsilon) = b \sim p(b|\theta)$ where $\epsilon \sim s(\epsilon)$. We assume that $f$ is a black-box or involves operations such that $\PT f = 0$ almost everywhere or is not computable.  Given the reparameterization, we can re-write this objective as an expectation over $\epsilon$ as  $\PT\E_{p(\epsilon)}[f(T(\epsilon, \theta)]$. We introduce an auxiliary, differentiable function approximator $r_\phi$ and note that:
\begin{align}
\PT\E_{p(b|\theta)}[f(b)] &= \E_{p(b|\theta)}\left[ \left[f(b) - r_\phi(b)\right]\cdot\LPT{b} + \PT r_\phi(b) \right]\nonumber\\
&= \E_{s(\epsilon)} \left[ \left[f(T(\epsilon, \theta)) - r_\phi(T(\epsilon, \theta))\right]\cdot\LPT{T(\epsilon, \theta)} + \PT r_{\phi}(T(\epsilon, \theta))\right]\nonumber
\end{align}
Which allows us to define our estimator below as: 

\begin{align}
\label{eq:cont_est}
\hat g_\textnormal{RELAX} = \left(f(b) -r_\phi(b)\right) \LPT{b} + \PT r_\phi(b) \qquad b = T(\epsilon, \theta), \epsilon \sim s(\epsilon).
\end{align}
