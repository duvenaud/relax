...
However, we expect this estimator to have high variance because the log-probability gradients in the control variate and the reinforce estimator are over different variables. The task of the control variate would be made much simpler if we could replace the $\PT \log p(z|\theta)$ in the control variate with $\PT \log p(b|\theta)$. This is the motivation behind our next estimator which we call RELAX.
...
\includegraphics[width=.31\textwidth]{figures/MNIST_L1} &
\includegraphics[width=.31\textwidth]{figures/MNIST_L2} &
\includegraphics[width=.33\textwidth]{figures/OMNIGLOT_L1} &
\includegraphics[width=.31\textwidth]{figures/OMNIGLOT_L2}\\
 \end{tabular}
\caption{Training curves for VAE Experiments. L1 represents the 1 layer linear model and L2 represents the 2 layer linear model. The horizontal dashed line indicates the lowest validation error obtained by REBAR.}
...
\setlength{\tabcolsep}{10pt}
\renewcommand{\arraystretch}{0}
\includegraphics[width=.32\linewidth]{figures/cp_paper} & 
\includegraphics[width=.31\linewidth]{figures/ll_paper} &
\includegraphics[width=.33\linewidth]{figures/ip_paper} & 
\includegraphics[width=.31\linewidth]{figures/idp_paper}\\
 \end{tabular}
...
\setlength{\tabcolsep}{10pt}
\renewcommand{\arraystretch}{0}
\includegraphics[width=.31\linewidth]{figures/cp_paper_var} & 
\includegraphics[width=.31\linewidth]{figures/ll_paper_var} &
\includegraphics[width=.31\linewidth]{figures/ip_paper_var} & 
\includegraphics[width=.29\linewidth]{figures/idp_paper_var}\\
 \end{tabular}
\caption{Variance of policy gradients (log scale) over 5 random seeds. The opaque bars indicate 1 standard deviation. After every 10th training episode 100 episodes were run and the sample log-variance is reported averaged over all policy parameters}
\label{fig:rl_var}
\end{figure}
