...
\section{Background: Building gradient estimators}
...
= \frac{\partial f}{\partial b} \times \frac{\partial b(\theta, \epsilon)}{\partial \theta}, 
...
We wish to compute $\PT\E_{p(b|\theta)}[f(b)]$ where $p(b|\theta)$ is a distribution with continuous support that admits a reparameterization $T$ such that $T(\theta, \epsilon) = b \sim p(b|\theta)$ where $\epsilon \sim p(\epsilon)$.
We assume that $f$ is a black-box or involves operations such that $\PT f = 0$ almost everywhere or is not computable.
Given the reparameerization, we can re-write this objective as an expectation over $\epsilon$ as  $\PT\E_{p(\epsilon)}[f(T(\epsilon, \theta)]$.
...
This estimator is unbiased for any control variate, but how can we tune the control variate to minimize its variance?
The answer is that we can use the gradient of the variance of our gradient estimator to tune its parameters $\phi$ with stochastic gradient-based optimization, at the same time as we optimize the parameters of our model or policy.
 
We can compute these gradients in at least two ways:
We could na\"ively differentiate through the empirical variance over some mini-batch.
Or, following \cite{tucker2017rebar}, we can construct an unbiased, single-sample estimator by using the fact that our gradient estimator is unbiased:
%
\begin{align}
...
= 2 g \frac{\partial g}{\partial \phi}.
\end{align}  % Do we need hats on these gs?  Or get rid of them elsewhere.

From inspection of the square of the estimator in \eqref{eq:cont_est} we can see that this loss also encourages $r(b)$ to approximate $f(b)$, but with a weighting based on $\PT\log p(b)$.  % Todo: reword this awkward sentence.
Moreover, as $r \rightarrow f$ then $\hat g_\textnormal{RELAX} \rightarrow \PT r$.
Thus, optimization in this way encourages a balance between the variance of the reparameterization estimator and the variance of the REINFORCE estimator. 
...
\begin{algorithm}[t]
	\caption{LAX Gradient Estimator for Continuous Random Variables}
	\label{discrete-relax}
	\begin{algorithmic}
		\Require function $f(\cdot)$, log-likelihood $\log p(\cdot; \theta)$, differentiable function approximator $r_\phi(\cdot)$,\\
		\hspace{3em} sampler $s(\epsilon)$, reparameterization $T(\cdot; \theta)$ 
		% sample s \sim p(s)
		\Function{$\hat{\mathcal{G}_i}$}{$\theta$}:
		\State $\epsilon_{i} \sim s(\epsilon)$
		\State $b_i \leftarrow T(\epsilon_i, \theta)$
		\State $\nabla_\theta \log p \leftarrow \texttt{autodiff}(\log p(b_i| \theta), \theta)$ 
		\State $\nabla_\theta r \leftarrow \texttt{autodiff}(r_\phi(b_i), \theta)$ 
		\State \textbf{return} $(f(b_i) - r_{\phi}(b_i))\times \nabla_\theta \log p + \nabla_\theta r$
		\EndFunction
		
		\Ensure $\frac{1}{K} \sum_{j=1}^K \hat{\mathcal{G}}_j(\theta)$
		\vspace{1.5mm}
	\end{algorithmic}
\end{algorithm}
...
We assume there exist a continuous distribution $p(z|\theta)$ and a determanistic mapping $H$ such that $H(z) = b \sim p(b|\theta)$.
If $p(b)$ is Bernoulli then $p(z|\theta) = \text{Logistic}(z|\theta)$ and $H(z) = \mathbb{I}(z>0)$.
If $b$ is categorical, then $p(z|\theta) = \texttt{softmax}(z|\theta)$ and $H(z) = \texttt{argmax}(z)$.
We also assume that $p(z|\theta)$ is reparameterizable.
Moreover, we assume the distribution $p(z|b, \theta)$ is also reparameterizable meaning there exists $\hat{T}$ such that $\hat{T}(\hat{\epsilon}, b, \theta) = \hat{z} \sim p(z|b, \theta)$ (see Appendix for details).


\section{Conditional reparameterization}
\label{conditional reparam section}
In this section, we incorporate a further refinement of our estimator, based on a technique due to \citet{tucker2017rebar}.
...
\end{align}
%
where $\sigma_\lambda$ is the \texttt{softmax} function with temperature $\lambda$. A similar relaxation for the Bernoulli distribution has been developed as well (see Appendix for details)
...
\paragraph{REBAR}
The recently-developed REBAR method \citep{tucker2017rebar} estimates the gradient of expectations of functions of Bernoulli random variables.
...
\subsection{The \RELAX{} estimator}
...
%\begin{align}
%\PT\E_{p(b|\theta)}[f(b)] \approx g_\phi(\epsilon,\hat{\epsilon}) &= \Big[f(b) - r_\phi(\hat{z})\Big]\cdot\LP{b} + \PT r_\phi(z) - \PT r(\hat{z})\\
...
We do not bootstrap the rewards with the value function.
...
In all experiments the model architectures were identical between our model and the baseline and the learning rates were also constant across tests making it that all improvements derive from having a lower variance estimate of the policy gradient. 
