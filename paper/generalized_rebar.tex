...
The back-propagation algorithm \citep{rumelhart1985learning}, also known as reverse-mode automatic differentiation [cite] computes exact gradients of deterministic, differentiable objective functions with a time cost comparable to evaluating the objective.
%Recent advancements such as stochastic variational inference \citep{hoffman2013stochastic} and 
The reparameterization trick \citep{kingma2013autoencoding, rezende2014stochastic} enabled the use of backpropagation to give unbiased, low-variance gradients estimates of expectations.
This has allowed the use of stochastic gradient-based fitting of large probabilistic latent-variable models.
...
In reinforcement learning, advantage actor-critic methods~\citep{mnih2016asynchronous} and deep deterministic policy gradients~\citep{lillicrap2015continuous} give low-variance but biased gradient estimates by jointly optimizing policy parameters as well as baselines or control variates.
In discrete latent-variable models, low-variance but biased gradient estimates can be given by continuous relaxations of discrete values~\citep{maddison2016concrete, jang2016categorical}.
...
In this paper, we simplify and generalize the method of \citet{tucker2017rebar} to learn a free-form control variate parameterized by a neural network, which gives lower-variance gradient estimates.
...
%There has been much recent work on producing gradient estimators for general computation graphs.
%In the case where all variables are continuous and deterministic, the backpropagation algorithm, also known as reverse-mode automatic differentiation, gives exact gradients.
