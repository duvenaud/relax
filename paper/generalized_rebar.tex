...
\begin{algorithm}[h]
	\begin{algorithmic}
		...
		\State $b_i \leftarrow T(\epsilon_i, \theta)$ \Comment Compute input
		\State $z_i \sim S(\epsilon_i, \theta)$ \Comment Compute unconditional relaxed input
		\State $\widetilde{z_i} \sim S(\widetilde{\epsilon_i}, \theta | b_i)$ \Comment Compute conditional relaxed input
		...
		\State \textbf{return} $\theta$ 
	\end{algorithmic}
	\caption{\RELAX{}: Low-variance contol variate optimization for black-box gradient estimation.}
	\label{lax}
\end{algorithm}
