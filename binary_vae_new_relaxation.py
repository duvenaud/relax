...
def CopyQFunc(z, name, reuse):
    ...
    with tf.variable_scope(name, reuse=reuse) as scope:
        ...
        out_r = tf.layers.dense(h2, 1, name="out")
...
class LearnedSampler:
    def __init__(self, u, q_func, name):
        self.u = u
        self.q_func = q_func
        self.name = name
        self.Qs = {}
...
def main(relaxation=None, learn_prior=True, max_iters=2000000,
    ...
    if relaxation is None:
        ...
        if relaxation == "copy":
            q_func = CopyQFunc
        elif relaxation == "individual":
            q_func = IndividualQFunc
        elif relaxation == "fc":
            q_func = FCQFunc
        else:
            assert False
        sig_z_sampler = LearnedSampler(u, q_func, "sig_z_sampler")
        sig_zt_sampler = LearnedSampler(v, q_func, "sig_zt_sampler")
