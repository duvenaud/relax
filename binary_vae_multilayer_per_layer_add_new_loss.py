...
def main(relaxation=None, learn_prior=True, max_iters=None,
    ...
    for l in range(num_layers):
        ...
        Q_objectives.append(tf.square(batch_f_b - batch_f_zt))
    ...
    q_gradvars = variance_opt.compute_gradients(Q_objective+wd, var_list=q_vars)
    q_train_op = variance_opt.apply_gradients(q_gradvars)
    variance_train_op = variance_opt.apply_gradients(variance_gradvars)
    model_train_op = model_opt.apply_gradients(model_gradvars)
    with tf.control_dependencies([model_train_op, variance_train_op, q_train_op]):
        train_op = tf.no_op()
