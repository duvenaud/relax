...
def Q_func(z, bs, x, name, reuse):
    with tf.variable_scope(name, reuse=reuse):
        input = tf.concat([2. * z - 1., x] + [2. * b - 1. for b in bs], 1)
        h1 = tf.layers.dense(input, 200, tf.nn.relu, name="q_1")
        h2 = tf.layers.dense(h1, 200, tf.nn.relu, name="q_2")
        #h3 = tf.layers.dense(h2, 200, tf.nn.relu, name="q_3")
        #h4 = tf.layers.dense(h3, 200, tf.nn.relu, name="q_4")
...
def main(use_reinforce=False, relaxed=False, learn_prior=True, num_epochs=820,
    ...
    TRAIN_DIR = "./binary_vae_relax_3_layer_sigmoid"
    ...
    sig_z_sampler = SIGZSampler(u, temperatures)
    sig_zt_sampler = SIGZSampler(v, temperatures)
    ...
    f_soft_z, _ = neg_elbo(x_binary, samples_z, inf_la_z, gen_la_z, p_prior)
    f_soft_zt, _ = neg_elbo(x_binary, samples_zt, inf_la_zt, gen_la_zt, p_prior)
    ...
    Q_objectives = []
    for l in range(num_layers):
        # sample sig(z) and sig(zt)
        log_alpha = inf_la_b[l]
        sig_z = sig_z_sampler.sample(log_alpha, l)
        sig_zt = sig_zt_sampler.sample(log_alpha, l)
        if l == 0:
            prev_b = []
        else:
            prev_b = samples_b[:l]

        name = "Q_func_{}".format(l)
        q_z = Q_func(sig_z, prev_b, x_binary, name, False)
        tf.summary.histogram("Q_{}".format(l), q_z)
        q_zt = Q_func(sig_zt, prev_b, x_binary, name, True)
        f_z = q_z + f_soft_z
        f_zt = q_zt + f_soft_zt
        f_zs.append(f_z)
        f_zts.append(f_zt)
        Q_objectives.append(tf.reduce_mean(tf.square(f_zt - f_b)))
    Q_objective = tf.add_n(Q_objectives)
    tf.summary.scalar("Q_objective", Q_objective)
    tf.summary.histogram("Q_sanity", f_zs[0] - f_zs[1])
    tf.summary.histogram("Q_sanity2", f_zs[1] - f_zs[2])

    f_b_mean = tf.reduce_mean(f_b)
    f_z_mean = tf.add_n([tf.reduce_mean(fz) for fz in f_zs])
    f_zt_mean = tf.add_n([tf.reduce_mean(fzt) for fzt in f_zts])
    ...
    grads = {}
    vals = [f_b_mean, f_z_mean, f_zt_mean, log_q_b]
    ...
    reinforces = []
    for l in range(num_layers):
        layer_params = [v for v in tf.global_variables() if "encoder" in v.name and layer_name(l) in v.name]
        for param in layer_params:
            ...
            reinforce = f_b_mean * d_log_q_dt + d_fb_dt
            rebar = (f_b_mean - eta * tf.reduce_mean(f_zts[l])) * d_log_q_dt + eta * (d_fz_dt - d_fzt_dt) + d_fb_dt
    ...
    Q_vars = [v for v in tf.global_variables() if "Q_func" in v.name]
    Q_gradvars = model_opt.compute_gradients(Q_objective, var_list=Q_vars)
    grad_vars.extend(Q_gradvars)

    for g, v in decoder_gradvars + Q_gradvars:
        print(v.name)
    ...
    tf.summary.scalar("fb", f_b_mean)
    for i, (fz, fzt) in enumerate(zip(f_zs, f_zts)):
        tf.summary.scalar("fz_{}".format(i), tf.reduce_mean(fz))
        tf.summary.scalar("fzt_{}".format(i), tf.reduce_mean(fzt))
    ...
    for i in range(iters):
        ...
        if i % 1000 == 0:
            loss, _, sum_str = sess.run([f_b_mean, train_op, summ_op], feed_dict={x: batch_xs})
            ...
            loss, _ = sess.run([f_b_mean, train_op], feed_dict={x: batch_xs})
        ...
        if i % iters_per_epoch == 0:
            ...
            for _ in range(dataset.test.num_examples // batch_size):
                ...
                losses.append(sess.run(f_b_mean, feed_dict={x: batch_xs}))
            tl = np.mean(losses)
